...
class AdDataService
{
    ...
    {
        if (!is_array($data)) {
            Log::warning('extractCampaigns: data is not an array', ['data' => $data]);
            return [];
        }
        if (isset($data['data']) && is_array($data['data'])) {
            Log::warning('extractCampaigns: data has data key', ['data' => $data['data']]);
            return $data['data'];
        }
        if ($this->isList($data)) {
            Log::warning('extractCampaigns: data is a list', ['data' => $data]);
            return $data;
        }
        return [];
    }
