    ...
    public function getAggregatedMetrics(): array
    {
        ...
        try {
            $response = Http::retry(
                ...
                when: function (Exception $excepton, $request){
                    // Only retry on:
                    // - Connection issues (timeouts, DNS failure, etc.)
                    // - HTTP 503 specifically
                    // - Or any 5xx server error if you want to be broader
                    return $exception instanceof ConnectionException ||
                        ($exception instanceof RequestException &&
                            $exception->response?->status() === 503);   // ← narrow to 503
                    // or $exception->response?->successful() === false && $exception->response?->serverError()
                },
                throw: true)
                ->timeout(15)
                ->withToken($token)
